...
export default function SideBar(props) {
  ...
  return (
    <Drawer variant="persistent" anchor="left" open={open} sx={styles.drawer} PaperProps={{ sx: styles.drawerPaper }}>
      ...
      {!keycloak.authenticated && open && (
        <Box
          ...
          mr="auto"
        >
          <Typography sx={styles.signInText}>Đăng nhập ngay để sử dụng những tính năng dành riêng cho bạn</Typography>
          <PrimaryButton onClick={() => keycloak.login()} style={{ width: 160, borderRadius: 25 }}>
            Đăng nhập
          </PrimaryButton>
        </Box>
      )}
      {/* </div> */}
      {image && <Background sx={styles.background} style={{ backgroundImage: "url(" + image + ")" }} />}
    </Drawer>
  );
}
