...
const ModalAddCategory = ({ isOpen, handleSuccess, handleClose }) => {
  ...
  const methods = useForm({
    resolver: zodResolver(
      z.object({
        type: z.string({ required_error: "This field is required" }).min(1, { message: "This field is required" }),
      })
    ),
    mode: "onChange",
  });
  ...
  const save = async (values) => {
    let successHandler = (res) => {
      ...
      successNoti("Add category successfully!", 3000);
      reset();
      handleClose();
    };
    ...
    request("post", `/targets/categories`, successHandler, errorHandlers, values);
  };
